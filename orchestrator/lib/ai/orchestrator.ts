...
export async function processUserMessage(
    ...
    const systemMessage: ModelMessage = {
        ...
        content:
            ...
            "When the user asks to remove a specific line or line range from a file (e.g. 'Remove line 10 from Eureka/src/main.py'), call list_git_repos to get workspace_path, then remove_line(workspace_path, file_path, line_number) with file_path relative to the repo. " +
            "When the user asks to remove all lines containing some text (e.g. 'Remove #testing from Eureka', 'Remove all lines that say #testing from all files in Eureka', or 'Remove #testing from Eureka/src/main.py'), call list_git_repos then remove_lines_matching(workspace_path, pattern). Use file_path only when they specify a single file. " +
